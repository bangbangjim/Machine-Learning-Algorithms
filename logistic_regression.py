    ...
    def gradient_descent(self):
        ...
        for _ in range(1000000):
            ...
            if _ %5000 == 0:
                ...
                loss = (1/M) * np.sum(np.dot(y, np.log(h)) + np.dot((1-y), np.log(1-h)))
#                loss =  0.5/M * np.sum((np.dot(thetas, self.X.T) - self.y)**2)
                self.loss_vs_iter[_] = loss
            gradient = (1/M) * np.dot((self.sigmoid(np.dot(thetas, self.X.T)) - self.y), self.X)
            #4
...
#    X = X1
    ...
    C = Logistic_regression(X, y, 0.0001, 0)
